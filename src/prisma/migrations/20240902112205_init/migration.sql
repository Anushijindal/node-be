-- CreateEnum
CREATE TYPE "ManagementTool" AS ENUM ('Trello', 'Zira');

-- CreateEnum
CREATE TYPE "RepositoryTool" AS ENUM ('GitHub', 'GitLab', 'BigBucket');

-- CreateEnum
CREATE TYPE "ProjectStatus" AS ENUM ('Development_Started', 'Launched', 'Coming_Soon');

-- CreateTable
CREATE TABLE "em_projects" (
    "project_id" SERIAL NOT NULL,
    "project_name" VARCHAR(20) NOT NULL,
    "project_technology" VARCHAR(30) NOT NULL,
    "project_start_date" TIMESTAMP(3) NOT NULL,
    "project_deadline_date" TIMESTAMP(3) NOT NULL,
    "project_lead" VARCHAR(20) NOT NULL,
    "project_manager" VARCHAR(20) NOT NULL,
    "project_client" VARCHAR(20) NOT NULL,
    "project_management_tool" "ManagementTool" NOT NULL,
    "project_management_tool_link" VARCHAR(60) NOT NULL,
    "project_repository_tool" "RepositoryTool" NOT NULL,
    "project_repository_tool_link" VARCHAR(60) NOT NULL,
    "project_description" VARCHAR(100) NOT NULL,
    "project_status" "ProjectStatus" NOT NULL,
    "project_updated_at" TIMESTAMP(3) NOT NULL,
    "project_deleted_at" TIMESTAMP(3) NOT NULL,
    "project_created_at" TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP,
    "project_user_id" INTEGER NOT NULL,

    CONSTRAINT "em_projects_pkey" PRIMARY KEY ("project_id")
);

-- CreateIndex
CREATE INDEX "em_projects_project_user_id_idx" ON "em_projects"("project_user_id");

-- AddForeignKey
ALTER TABLE "em_projects" ADD CONSTRAINT "em_projects_project_user_id_fkey" FOREIGN KEY ("project_user_id") REFERENCES "em_users"("user_id") ON DELETE RESTRICT ON UPDATE CASCADE;
